# MetaFeeder · Papers — Rationale

## What deviation / exception is being requested

`metafeederpaper-feeder` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  `/DATA/AppData/metafeederpaper/`, declared in `x-compose-app.folders` and owned `$PUID:$PGID`.
...
(where applicable) the TMDB cache — lives under `/DATA/AppData/metafeederpaper/`, which is the unit
