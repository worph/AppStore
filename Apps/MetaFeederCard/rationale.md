# MetaFeeder · Cards — Rationale

## What deviation / exception is being requested

`metafeedercard-feeder` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  `/DATA/AppData/metafeedercard/`, declared in `x-compose-app.folders` and owned `$PUID:$PGID`.
...
(where applicable) the TMDB cache — lives under `/DATA/AppData/metafeedercard/`, which is the unit
